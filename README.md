...
### judge-core
`scratch-debugger-gui` uses [`judge-core`](https://github.ugent.be/scratch4d/judge-core) as local `npm` package.

Clone the `judge-core` repository in the same directory where you clone(d) `scratch-debugger-gui`.
```bash
git clone git@github.ugent.be:scratch4d/judge-core.git
```

Install it's dependencies and build the `judge-core` package.
```bash
cd judge-core
npm install
npm run build
...
Every change in `judge-core` needs a rebuild with `npm run build` for the changes to take effect in `scratch-debugger-gui`.
...
### scratch-debugger-gui
Clone the `scratch-debugger-gui`-repository in the same folder as where the `judge-core`-repository is.
...
Finally, execute the following command in the root of the repository. This makes sure the local `scratch-vm` is being used, in contrary to the one from the `npm` repositories.
´´´
npm link scratch-vm
´´´
